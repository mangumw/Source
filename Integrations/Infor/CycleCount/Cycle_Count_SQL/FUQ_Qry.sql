...
SELECT a.BatchNumber
	, a.SKU
	, a.StoreNumber
	, MIN(a.BatchPass) as Initial_Pass
	, MAX(a.BatchPass) as Variance_Pass
	, CAST(MIN(a.Scan_date) as DATE) as Initial_Scan_Date
	, CAST(MAX(a.Scan_date) as DATE) as Variance_Scan_Date
	, CAST(MIN(a.Date_Key) as DATE) as Initial_Date
	, CAST(MAX(a.Date_Key) as DATE) as Variance_Date
	, COUNT(DISTINCT a.BatchPass) as Number_of_Counts
FROM CTE a
GROUP BY a.BatchNumber, a.SKU, a.StoreNumber
),
CTE3 as
(
...
SELECT DISTINCT 
	...
	, c.MMS_Qty
	, c.Count_Qty
	, [Difference] = c.MMS_Qty - c.Count_Qty
